...
class BaselineTests extends AnyFunSuite with BeforeAndAfterAll {
   ...
   test("Compute global average")                           { assert(inRange(global_average_rating_q1(train2), 1.0, 5.0)) }
   test("Compute user 1 average")                           { assert(inRange(user_1_average_rating(train2), 1.0, 5.0)) }
   test("Compute item 1 average")                           { assert(inRange(item_1_average_rating(train2), 1.0, 5.0))  }
   test("Compute item 1 average deviation")                 { assert(inRange(item_1_deviation(train2), 0.0, 1.0))  } // ??? 
   test("Compute baseline prediction for user 1 on item 1") { assert(inRange(baseline_user1_item1(train2), 1.0, 5.0)) }
   ...
   test("MAE on all four non-personalized methods on data/ml-100k/u2.base and data/ml-100k/u2.test") {
     assert(global_average_method(train2,test2).filter{x => (!inRange(x,1.0,5.0))}.size == 0)
     assert(user_average_method(train2,test2).filter{x=> (!inRange(x,1.0,5.0))}.size == 0)
     assert(item_average_method(train2,test2).filter{x=> (!inRange(x,1.0,5.0))}.size == 0)
     assert(baseline_method(train2,test2).filter{x=> (!inRange(x,1.0,5.0))}.size == 0)

     // MAE should be in this order : Global average > User average > Item Average > Baseline Method
     assert(compute_MAE_by_predictor(train2,test2,global_average_method) > compute_MAE_by_predictor(train2,test2,user_average_method))
     assert(compute_MAE_by_predictor(train2,test2,user_average_method) > compute_MAE_by_predictor(train2,test2,item_average_method))
     assert(compute_MAE_by_predictor(train2,test2,item_average_method) > compute_MAE_by_predictor(train2,test2,baseline_method))
   }
